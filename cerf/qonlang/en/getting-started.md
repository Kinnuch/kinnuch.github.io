...
3. **Recent projects**: four per row, at most two rows (eight); one card per project you opened recently, with its name and author, and a word **written in that project's own script** at the bottom right (files are read one at a time, so the cards appear first and the writing follows; whichever end overflows fades out, downwards for vertical scripts). Hovering a card lights it with a sheen that follows the pointer; click to open it.
4. **Games**: a row of four games under the recent projects, all built from your own lexicon. Click one, pick a project (read only — it is **not opened and not changed**), then pick a language:
...
5. **Recent**: the list at the bottom of the left column; files that no longer exist are removed from it automatically.

The left column also has New project, Open project, **Example projects** and User guide. **Example projects** contains two fictional projects that, between them, show every module — the quickest way to see what a feature looks like and how its data should be filled in:
...
- Buttons for saving (**Make a copy** for an example project, see above), showing the inspector and closing the project.
