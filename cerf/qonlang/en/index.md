...
- **Three columns**: navigation on the left, the main area in the middle, the **Inspector** on the right. Drag the divider to resize the inspector. Clicking the current module again in the navigation bar keeps you where you are; clicking Skin or Settings again returns to the previous page.
...
- **Right-click menu**: anywhere in the app — cut / copy / paste, undo / redo / back, command palette, character panel, save and user guide.
...
- **Aelith**: an agglutinative language — a proto-language → modern language family with a sister language, Merun (open the relation graph of kaso or teli and click **Compare** to see how the cognates' sound changes and meanings differ), vowel harmony, multi-slot suffixes and every kind of pipeline step, variants (the base set renamed to 书面, "written") and inheritance, sandhi voicing that applies to all words, compound parts of speech and per-sense parts of speech, inspector modules (a cultural note, and runic variants shown in the runic font), etymology chains and the relation graph, a runic script and glossed, confirmed example sentences (the personal name Mira is deliberately missing from the lexicon, so its hover card says "not found").
- **Tsahun**: an isolating tone language — tones, two orthographies, syllabary packing and vertical text, bracket settings in the script, `@entry` references producing inflected forms with spaces (`ngo21 tui55`), candidates for the homograph hok33, and a "variant characters" module shown in the syllabary font.
