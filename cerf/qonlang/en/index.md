...
- **Three columns**: navigation on the left, the main area in the middle, the **Inspector** on the right. By default the inspector width follows the window (the main area keeps enough width for its content, the rest goes to the inspector); drag the divider to set your own width, and double-click it to go back. Clicking the current module again in the navigation bar keeps you where you are; clicking Theme or Settings again returns to the previous page.
- **Grey hints inside boxes**: when an inspector box is too narrow for its hint, hover over it and the full hint appears as a tooltip.
...
- **Everything is project data**: parts of speech, dimensions, abbreviations, dialects and export templates are all stored in the project file, so they travel with it to another computer. The theme and the interface language are application settings and are not stored in the project.
...
- **Collapsible sections**: long sections (parts of speech & dimensions, the phoneme and vowel tables, syllable packing and auto-mapping in Script, the entry card and font library in Theme…) have a triangle next to their title that appears on hover; click it to collapse the whole section, and it stays collapsed next time. In Parts of speech & dimensions, each part of speech, dimension and inspector module can also be collapsed to a single line.
- **Long lists stay fast**: the lexicon draws only the few dozen rows in view and follows as you scroll, keeping the scroll position (row heights snap to whole device pixels, so the table doesn't go blurry however deep you scroll); morphemes, corpus, paradigm slots and the glyph table draw one screenful first and the next batch as you near the bottom, so switching pages and changing filters stays fast.
- **The first time you open the app**, the start page runs a tour (new / open, starter templates, recent projects, the mini-games, the language and settings buttons at the bottom left, the row along the bottom) and ends on **Rule syntax**; to see a tour again later, click **User guide** next to a page title. All twelve modules have one, plus **Plugins and MCP**.
...
- **Test bench**: below the sample sits a **Test bench** (collapsed by default — click the heading to open it). Type input of your own — one data row, a few rules, one glyph mapping line — and it is parsed with the current import settings and shown exactly like the sample. It saves changing the file and re-importing just to find out whether a column is mapped right or a marker is recognised. Changing a setting recomputes it, and what you typed is still there when you come back to the page.
...
The **Example projects** button on the start page lists two fictional language projects: open one and change anything, but it can't be saved — use **Make a copy** to keep one (see [Getting started · The start page](/cerf/qonlang/en/getting-started/#2-the-start-page)). They are two completely different language types that, together, show every module:

- **Aelith**: an agglutinative language — a proto-language → modern language family with a sister language, Merun (open the relation graph of kaso or teli and click **Compare** to see how the cognates' sound changes and meanings differ), vowel harmony, multi-slot suffixes and every kind of pipeline step, variants (the base set renamed to 书面, "written") and inheritance, sandhi voicing that applies to all words (a tweak step that declares its own pair of classes, `{清塞}=p t k` and `{浊塞}=b d g`), compound parts of speech and per-sense parts of speech, inspector modules (a cultural note, and runic variants shown in the runic font), etymology chains and the relation graph, features, stress rules and the syllable boundary σ (the 书面语 → 口语 rule set, "written → spoken", where every column of the test bench carries ˈ; its last rule turns both t and d into r between vowels, with r written twice), Affects stress (pronouns and particles pass their part of speech and stay unstressed; telikaso has special stress and reads teliˈkaso), a custom stress rule for the sister language Merun (where hara has special stress), the entry vesa deliberately left without a definition (reachable from the lexicon's status bar), a runic script with a hand-drawn glyph (the "notch full stop"), a paradigm inside a paradigm (the Gerund paradigm adds -mAk and then runs the Noun cases: sörmek, sörmekde — sör- uses it as an extra paradigm next to its conjugation), the entry sörmek created from that gerund with its etymology and relation filled in automatically, the family node "Ael 语系" (click it for counts, phonemes, cognate rates and correspondences), two historical stages inside Aelith, CAe → Ae, with each entry's history (kaso shows PAe kasu → CAe kaso → Ae kaso), plural noun cases based on 复数.主格 (plural nominative) that only add the case suffix, locative slots that affect pronunciation (kasoda reads ˈkasoða), an allomorph picked by a grammatical value (the plural marker is -lAn before the dative: kasolanka), the verb's third-person cells deliberately left empty so that in simple mode they continue from the 极性.时 (polarity-tense) slots, and glossed, confirmed example sentences with punctuation (the personal name Mira is deliberately missing from the lexicon, so its hover card says "not found").
- **Tsahun**: an isolating tone language — tones, two orthographies (phonemes have their romanised spellings filled in, so ts and ng count as one sound: the intensive reduplication of tsing55 is tsitsing55), syllabary packing and vertical text, bracket settings in the script, `@entry` references producing inflected forms with spaces (`ngo21 tui55`), candidates for the homograph hok33, a "variant characters" module shown in the syllabary font, and a logographic script, Tsahun 刻符, written word by word from glyph codes (script lines in the corpus and phrasebook look each word up in the lexicon; particles keep their spelling).
