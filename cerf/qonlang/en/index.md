...
- **Long lists stay fast**: the lexicon draws only the few dozen rows in view and follows as you scroll, keeping the scroll position; morphemes, corpus, paradigm slots and the glyph table draw one screenful first and the next batch as you near the bottom, so switching pages and changing filters stays fast.
