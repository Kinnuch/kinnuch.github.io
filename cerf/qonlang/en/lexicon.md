...
- Search headwords, definitions, inflected forms and pronunciations; or search a single field: `word=` (headword), `gloss=` (definition), `form=` (inflected form), `stem=` (stem), `ipa=` (pronunciation), `pos=` (part of speech), `tag=` (tag), `register=` (register), `etym=` (etymology), `note=` (notes), `script=` (script), `feature=` (a grammatical dimension value — a dimension's name works as a field too, e.g. `gender=fem`), `dialect=` (dialect), or `/regex/` — see [Common conventions](/cerf/qonlang/en/#common-conventions). Header sorting and funnel filters are also described in [Common conventions](/cerf/qonlang/en/#common-conventions); the filterable columns here are initial letter, part of speech, tags, each dimension and dialects, plus language for **All languages**. To sort by last modification, tick **Updated** under **Columns** first and then click its column name.
- **Columns** chooses which columns are shown: part of speech, each gloss language, tags, etymology, pronunciation, each dimension, dialects (when a language defines dialects), each stem slot, each inflected form, each script, and the update time. The choice is saved in the project settings. If columns from different sources end up with the same name, a notice above the list suggests renaming them at their source.
- Column widths **fill** the table area, so there is no empty space on the right: columns you haven't resized get a default width by kind (definitions wider, part of speech and dimensions narrower); adding a column first shares out the existing columns' widths proportionally, never below a minimum, and only scrolls horizontally when they really don't fit. The right edge of each column can be **dragged to resize** — the neighbouring column on the right gives or takes the difference. Widths are stored in your local preferences, and **Reset column widths** is at the bottom of the column menu.
...
- **Favourites**: each row has a star at its far left. It stays invisible until you hover the row, and one click marks the entry as a favourite (the star fills in yellow and stays visible). The star button at the top right of the page is **Favourites only** — one click narrows the list to your favourites, another restores it. Favourites are stored in the project, and the same star sits next to **Word** in Edit mode.
- **Marks**: tick **Mark** next to **Word** in Edit mode and the entry gets a symbol in front of it in the list and on its card (`*` by default) — a way to flag a handful of words as important. The symbol is set in [Settings](/cerf/qonlang/en/settings/) → Current project → Basics → **Mark symbol**, and changing it changes every marked word at once.
...
**The word is highlighted where it occurs**: a whole-word match highlights the whole word; when the match is an inflected form, or only one piece of a compound or affixed word, just that piece is highlighted. When the position can't be pinned down (the text was edited by hand, the analysis is out of date), the sentence still counts as an example, it just isn't highlighted.

With **Examples under an entry also show the script line and where they come from** ticked in [Settings](/cerf/qonlang/en/settings/) → Application → Display, each example gains a line above it with the language's scripts (written as on the Corpus page) and its **source** after it; it is off by default.
...
| `stress`, `对重音影响`, `重音` | Affects stress | `pos` passes the part of speech; `special:-2` (or just a number; `倒数第2` and `第1` work too) passes special stress, with `0` or `unstressed` meaning unstressed; separate items with commas, e.g. `pos, special:-1`. The column is exported only when some entry has it ticked |
...
| `tags`, `标签` | Tags | |
| `notes`, `备注` | Notes | |
| `stress`, `对重音影响` | Affects stress | As for entries, plus `as:noun` (`算作:名词`) for the part of speech it counts as |
