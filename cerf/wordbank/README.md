...
核心是 SM-2，但有三处按中文背单词 App 的实际手感做了改动：
...
2. **只有三档评价**（不认识 / 有印象 / 认识，对应 q = 0 / 3 / 5）。选择题和拼写题
   自动判分；答对后仍可点「其实有点模糊」降级。
3. **「有印象」从不拉长间隔。**不管原来隔了多久，一律退回阶梯底部（1–2 天后再见），
   熟练度系数按 SM-2 下调 0.14。早期版本是「原间隔 × 1.25」，于是一个点过「已认识」
   （直接记为 60 天）的词，下次只要犹豫一下答「有印象」，就会被推到 83 天后 ——
   犹豫本来说明间隔太长，结果却推得更远。

答错则熟练度系数 −0.2、次数归零、当场重新排进本轮队列（隔 4~6 张卡再出现）。
复习间隔超过「掌握阈值」（默认 60 天）的词标记为已掌握。

**抽查**是例外：从学过的词里随机抽 20 个。答「认识」或选对**不**拉长间隔（提前答对不
构成新证据）；答「有印象」或答错则照常排期 —— 犹豫和遗忘都说明间隔太长，该拉回来。
所以抽查只会让词离你更近，不会把词推远。设置里可以让抽查**只用自评卡** —— 不给选项、
不给提示，只有单词，适合考前自查。
...
揭晓答案的页面（自评卡选完之后、选择题 / 拼写答完之后）底部有**收藏**按钮。收藏的词
可以在首页「只抽收藏」里单独抽查，也能在词库里筛出来。收藏不算作答的一部分，点「上一题」
撤回作答时不会把收藏一起撤掉。
...
新版本发布后，下次打开应用时新的 Service Worker 一接管就自动刷新一次页面，不用开两次
才看到更新；如果正在学习，会等本轮结束再刷新，不打断作答。
...
node tools/smoke-srs.mjs                                    # 排期规则（不需服务器）
node tools/smoke-lookup.mjs                                 # 查词与词形还原（不需服务器）
node tools/smoke-import.mjs                                 # 真实词表解析 + 查词（不需服务器）
npm i puppeteer-core && node tools/smoke.mjs                # 浏览器端到端（62 项）
node tools/smoke-books.mjs                                  # 内置词书导入（10 项）
```

前三个是纯 Node 的，跑得很快：`smoke-srs.mjs` 单独验证排期规则（阶梯、「有印象」必定
拉回、不认识清零）；`smoke-lookup.mjs` 直接读 `data/dict.json`，覆盖单词、
